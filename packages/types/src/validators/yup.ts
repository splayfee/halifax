...
/** The subset of yup's `SchemaDescription` shape this converter reads (version-stable fields). */
...
function applyConstraints(out: JsonSchema, desc: YupDescription): JsonSchema {
  if (Array.isArray(desc.oneOf) && desc.oneOf.length > 0) out.enum = [...desc.oneOf]
  ...
  for (const test of desc.tests ?? []) {
    ...
    switch (test.name) {
      case 'integer':
        ...
        break
      case 'min':
        if (typeof p.min === 'number') out[out.type === 'string' ? 'minLength' : 'minimum'] = p.min
        break
      case 'max':
        if (typeof p.max === 'number') out[out.type === 'string' ? 'maxLength' : 'maximum'] = p.max
        break
      case 'length':
      ...
      case 'uuid':
        ...
        break
      case 'matches':
        if (p.regex instanceof RegExp) out.pattern = p.regex.source
        break
      default:
        break
    }
  ...
  if (desc.nullable && typeof out.type === 'string') out.type = [out.type, 'null']
  return out
}

/** Recursively converts a yup schema description into a JSON Schema node. */
function descriptionToJsonSchema(desc: YupDescription): JsonSchema {
  switch (desc.type) {
    case 'object': {
      ...
      for (const [key, field] of Object.entries(desc.fields ?? {})) {
        ...
        if (field.optional === false) required.push(key)
      }
    ...
    case 'array':
      return applyConstraints(
        { type: 'array', items: desc.innerType ? descriptionToJsonSchema(desc.innerType) : {} },
        desc
      )
    case 'string':
 ...
 */
export function yupValidator<T>(schema: yup.Schema<T>): ISchemaValidator<T> {
  return {
    async validate(data: unknown): Promise<ValidationResult<T>> {
      try {
        ...
        if (err instanceof yup.ValidationError) {
          ...
          const errors: FieldError[] = source.map((e) => ({
            path: e.path ?? '',
            message: e.message
          }))
    ...
    toJsonSchema(): JsonSchema | undefined {
      try {
        return descriptionToJsonSchema(schema.describe() as unknown as YupDescription)
      } catch {
