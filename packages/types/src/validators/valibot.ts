...
/** The subset of a Valibot schema node this converter walks (stable v1 structural fields). */
interface VNode {
  kind?: string
  type?: string
  ...
  options?: unknown[]
  literal?: unknown
  pipe?: Array<{ kind?: string; type?: string; requirement?: unknown }>
}
...
function isOptional(node: VNode): boolean {
...
function applyPipe(out: JsonSchema, pipe: VNode['pipe']): void {
  for (const item of pipe ?? []) {
    ...
    switch (item.type) {
      case 'integer':
        ...
        break
      case 'min_value':
        if (typeof req === 'number') out.minimum = req
        break
      case 'max_value':
        if (typeof req === 'number') out.maximum = req
        break
      case 'min_length':
        if (typeof req === 'number') out.minLength = req
        break
      case 'max_length':
        if (typeof req === 'number') out.maxLength = req
        break
      case 'length':
      ...
      case 'uuid':
        ...
        break
      case 'regex':
        if (req instanceof RegExp) out.pattern = req.source
        break
      default:
        break
    }
...
function nodeToJsonSchema(node: VNode): JsonSchema {
  ...
  while (
    ...
    if (NULLABLE_TYPES.has(current.type ?? '')) nullable = true
 ...
 */
export function valibotValidator<T>(schema: v.GenericSchema<unknown, T>): ISchemaValidator<T> {
  return {
    ...
    toJsonSchema(): JsonSchema | undefined {
      try {
        return nodeToJsonSchema(schema as unknown as VNode)
      } catch {
        return undefined
      }
    }
