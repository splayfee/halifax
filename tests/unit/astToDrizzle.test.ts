...
describe('astToDrizzleWhere — empty input', () => {
  ...
  it('skips filters for unknown columns (returns undefined for that node)', () => {
    ...
    const result = astToDrizzleWhere(
      [{ field: 'nonexistent', comparison: SqlComparison.Equal, value1: 'x' }],
      cols
    )
...
describe('astToDrizzleWhere — comparison operators', () => {
  it('NotEqual (<>)', () => {
    expect(query({ field: 'name', comparison: SqlComparison.NotEqual, value1: 'Alpha' })).toEqual([
      'Bravo',
      'Charlie'
    ])
  })
  ...
  it('GreaterThan (>)', () => {
    expect(query({ field: 'score', comparison: SqlComparison.GreaterThan, value1: 10 })).toEqual([
      'Bravo',
      'Charlie'
    ])
  })
  ...
  it('LessThan (<)', () => {
    expect(query({ field: 'score', comparison: SqlComparison.LessThan, value1: 30 })).toEqual([
      'Alpha',
      'Bravo'
    ])
  })
  ...
  it('LessThanOrEqual (<=)', () => {
    expect(
      query({ field: 'score', comparison: SqlComparison.LessThanOrEqual, value1: 20 })
    ).toEqual(['Alpha', 'Bravo'])
  })

  it('Between (inclusive)', () => {
    expect(
      query({ field: 'score', comparison: SqlComparison.Between, value1: 10, value2: 20 })
    ).toEqual(['Alpha', 'Bravo'])
  })

  it('NotBetween', () => {
    expect(
      query({ field: 'score', comparison: SqlComparison.NotBetween, value1: 15, value2: 25 })
    ).toEqual(['Alpha', 'Charlie'])
  })

  it('NotIn', () => {
    expect(
      query({ field: 'name', comparison: SqlComparison.NotIn, value1: ['Alpha', 'Bravo'] })
    ).toEqual(['Charlie'])
  })
  ...
  it('NotLike', () => {
    expect(query({ field: 'name', comparison: SqlComparison.NotLike, value1: 'Al%' })).toEqual([
      'Bravo',
      'Charlie'
    ])
  })
  ...
  it('EndsWith', () => {
    expect(query({ field: 'name', comparison: SqlComparison.EndsWith, value1: 'ie' })).toEqual([
      'Charlie'
    ])
  })
