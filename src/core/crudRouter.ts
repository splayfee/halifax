...
export function normalizeError(error: unknown): { status: number; name: string; message: string; details?: unknown } {
  if (error instanceof HttpError) {
    return { status: error.status, name: error.name, message: error.message, details: error.details }
  }
...
async function authorizeRequest(
  ...
  if (authStrategy.authorize) {
    const allowed = await authStrategy.authorize({ auth, action, resource, requiredPermissions, req })
...
export function registerCrudApi(server: HttpServer, resources: ResourceDefinition[], options: CrudApiOptions = {}): void {
  ...
  resources.forEach((resource) => {
    ...
    if (!repository) throw new HttpError(`Resource '${resource.name}' does not define a repository.`, 500)
    ...
    if (permissions.allowReadManyWithQueryBuilder) {
      server.registerRoute('POST', `${basePath}/${queryBuilderPath}`, wrap(async (req, res) => {
        ...
        if (!repository.executeQueryBuilder) throw new HttpError('This resource does not support the query builder.', 501)
        const body = (req.body ?? {}) as Record<string, unknown>
        const query = { ...body, tableName: body.tableName ?? resource.tableName } as IQueryOptions
    ...
    if (permissions.allowUpdateMany) {
      server.registerRoute('PATCH', basePath, wrap(async (req, res) => {
        ...
        if (!repository.updateMany) throw new HttpError('This resource does not support updateMany.', 501)
        const { update, ...queryBody } = (req.body ?? {}) as Record<string, unknown>
        const query = { ...queryBody, tableName: queryBody.tableName ?? resource.tableName } as IQueryOptions
        validateAdvancedQuery(resource, query)
    ...
    if (permissions.allowDeleteMany) {
      server.registerRoute('DELETE', basePath, wrap(async (req, res) => {
        ...
        if (!repository.deleteMany) throw new HttpError('This resource does not support deleteMany.', 501)
        const body = (req.body ?? {}) as Record<string, unknown>
        const query = { ...body, tableName: body.tableName ?? resource.tableName } as IQueryOptions
        validateAdvancedQuery(resource, query)
  ...
  server.registerRoute('POST', previewPath, async (req, res) => {
    ...
    await res.status(200).json({ count: QueryBuilder.buildCountQuery(query), select: QueryBuilder.buildSelectQuery(query) })
  })
}
