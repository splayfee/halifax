...
function addCreate(rc: ResolverContext, mutationFields: MutationFields): void {
  ...
  mutationFields[`createMany${typeName}`] = {
    ...
    args: { input: { type: new GraphQLNonNull(new GraphQLList(new GraphQLNonNull(types.CreateInputType))) } },
    async resolve(_parent, args: { input: Record<string, unknown>[] }, context: GraphQLResolverContext): Promise<unknown> {
      try {
        const auth = await authorizeRequest(context.req, resource, 'create', authStrategy)
        const repo = await resolveRepo(context.req, auth, 'create')
...
function addUpdateOne(rc: ResolverContext, mutationFields: MutationFields): void {
  ...
  mutationFields[`update${typeName}`] = {
    ...
    args: { id: { type: new GraphQLNonNull(GraphQLID) }, input: { type: new GraphQLNonNull(types.UpdateInputType) } },
...
function addUpdateMany(rc: ResolverContext, mutationFields: MutationFields): void {
  ...
  mutationFields[`updateMany${typeName}`] = {
    ...
    args: {
      where: { type: new GraphQLNonNull(new GraphQLList(new GraphQLNonNull(helpers.QueryFilterInput))), description: 'At least one filter is required to prevent accidental full-table updates.' },
      update: { type: new GraphQLNonNull(types.UpdateInputType), description: 'Fields to apply to every matched record.' }
    },
...
function addUpsert(rc: ResolverContext, mutationFields: MutationFields): void {
  ...
  mutationFields[`upsert${typeName}`] = {
    ...
    args: { id: { type: new GraphQLNonNull(GraphQLID) }, input: { type: new GraphQLNonNull(types.CreateInputType) } },
    async resolve(_parent, args: { id: string; input: Record<string, unknown> }, context: GraphQLResolverContext): Promise<unknown> {
      try {
...
function addDeleteMany(rc: ResolverContext, mutationFields: MutationFields): void {
  ...
  mutationFields[`deleteMany${typeName}`] = {
    ...
    args: {
      where: { type: new GraphQLNonNull(new GraphQLList(new GraphQLNonNull(helpers.QueryFilterInput))), description: 'At least one filter is required to prevent accidental full-table deletes.' }
    },
