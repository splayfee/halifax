...
const httpStatusToGqlCode: Record<number, string> = {
  400: 'BAD_REQUEST',
  401: 'UNAUTHORIZED',
  403: 'FORBIDDEN',
  404: 'NOT_FOUND',
...
export function buildGraphQLSchema(contexts: GraphQLResourceContext[]): GraphQLSchema {
  ...
  for (const { resource, authStrategy, hooks, resolveRepo } of contexts) {
    if (resource.graphql === false) continue
    ...
    const OutputType: GraphQLObjectType = new GraphQLObjectType({
      name: typeName,
    ...
    if (permissions.allowReadMany) {
      queryFields[`list${typeName}`] = {
        ...
        description:
          `Fetch a paginated list of ${typeName} records. ` +
        ...
        async resolve(
          ...
          try {
            ...
            const processedOptions = await applyHook(
              hooks?.beforeReadMany,
              listOptions,
              hookCtx
            )
    ...
    if (permissions.allowReadManyWithQueryBuilder) {
      queryFields[`query${typeName}`] = {
        type: new GraphQLNonNull(ListResultType),
        description:
          `Advanced query for ${typeName} with full filter expressions, sorting, and pagination. ` +
          ...
          try {
            ...
            const query: IQueryOptions = {
              ...(args.where !== undefined
                ? { where: args.where as unknown as IQueryFilter[] }
                : {}),
